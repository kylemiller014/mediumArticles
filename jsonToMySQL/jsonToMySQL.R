...
)
{
  ...
  getAllOtherColNames <- paste0("SHOW columns FROM ", schemaTable, " where `Key` != 'PRI'
    ...
    for(j in 1:endRow){
      ...
      for(k in 1:nrow(allOtherColDf)){
        ...
        forFancyQuotes <- append(forFancyQuotes,
                                 eval(parse(text = paste0("sQuote(df[j,", k,"],
...
# Testing….
weatherJsonParse("127.0.0.1", 3306,"kyle_dba", "weather","atlanta_cur",
                 "/Users/kylemiller/Medium Articles/genericJsonParse/",
                 1,"http://api.weatherapi.com/v1/current.json?")